...
rankReference = []
Bests = []
with open('fullstats2.csv','r') as filegreader:
...
with open('fullstats.csv', 'r') as original:
                ...
                with open(teamcsv,'r') as readme:
                    ...
                    for game in filereading:
                        ...
                        if len(game) > 0:
                            ...
                            #########################
                            if opponentRank > rank:#away == False
                                bigOffense += 1
                                if outcome == 'W':
                                    WinEX += 1
                                else:
                                    LoseU += 1
                            if  opponentRank < rank:# away == True and
                                bigDefense += 1
                                if outcome == 'W':
                                    print(opponent,opponentRank)
                                    WinU += 1
                                else:
                                    LoseEX += 1

            if bigDefense != 0:
                WinU = WinU / gamesPlayed
                LoseEX = LoseEX / gamesPlayed
            else:
                WinU = 0
                LoseEX = 0
            if bigOffense != 0:
                WinEX = WinEX / gamesPlayed
                LoseU = LoseU / gamesPlayed
            else:
                WinEX = 0
                LoseU = 0
            newstats = [rank,name,WinEX,LoseEX,WinU,LoseU]

            # with open('RankOnlyStats.csv','a') as latest:
